# Modified from: https://github.com/Kitware/sprokit/blob/master/conf/sprokit-macro-configure.cmake
#
# Configure functions for the rrm project
# The following functions are defined:
#
#   rrm_configure_file
#   rrm_configure_directory
#
# The following variables may be used to control the behavior of the functions:
#
#   rrm_configure_extra_dests
#     A list of other paths to configure the file into.
#
#   rrm_configure_cmake_args
#     Extra arguments to pass to CMake when running the generated script.
#
# Their syntax is:
#
#   rrm_configure_file(name source dest [variable ...])
...
#
#   rrm_configure_directory(name sourcedir destdir)
...
function (_rrm_configure_file name source dest)
...
configure_file(
  ...
  foreach (extra_dest IN LISTS rrm_configure_extra_dests)
    file(APPEND "${configure_script}" "
configure_file(
  \"${configured_path}\"
  \"${extra_dest}\"
...
function (rrm_configure_file_always name source dest)
  set(extra_output
    "${dest}.noexist")

  rrm_configure_file(${name} "${source}" "${dest}" ${ARGN})
endfunction ()

function (rrm_configure_directory name sourcedir destdir)
