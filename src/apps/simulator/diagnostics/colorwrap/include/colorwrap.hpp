...
class COLORWRAPLIB_DLL_HANDLER View
{
    public:
        View(std::vector<int> colormap) : colormap_(colormap) {}
        ~View() = default;
        ...
        std::size_t numColors() { return colormap_.size(); };

        template<typename T = int>
        std::array<T, 3> rgb(std::size_t i)
        {
            std::array<T, 3> color{};
            auto& m = colormap_;
            color = {T(m[3*i + 0]), T(m[3*i + 1]), T(m[3*i + 2])};
            return color;
        }

    private:
        std::vector<int> colormap_;
};
