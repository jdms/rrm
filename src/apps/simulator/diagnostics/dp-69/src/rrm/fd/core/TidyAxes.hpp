...
namespace rrm::fd::core
{
  ...
  class FaceProperties
  {
    ...
    void Init(vtkRenderer* renderer, dpl::vector3d color, float line_width) {
    ...
    void BuildGridlines(dpl::vector3d min, dpl::vector3d step, dpl::vector3i count) {
      points_->Initialize();
      lines_->Initialize();
    ...
    void RefreshVisibility() {
      const dpl::vector2i_map viewport_size{viewport_->GetSize()};
      ...
      if (auto* camera = viewport_->GetActiveCamera();
        (dpl::vector3d_map{camera->GetFocalPoint()} - dpl::vector3d_map{camera->GetPosition()})
      ...
      dpl::vector2d face_triangle_display[3];
      ...
      //   //   dpl::vector3d{coord->GetComputedWorldValue(viewport_)};
      //   //   face_triangle_display[0];
      //
      //     auto* cmr = viewport_->GetActiveCamera();
      //
      //     dpl::vector3d_map pos{cmr->GetPosition()};
      //   dpl::vector3d_map focal{cmr->GetFocalPoint()};
      //   
  ...
  class TidyAxes
  {
    ...
    static dpl::vector2i GetAlignment(dpl::vector2d v) {
    ...
    void RefreshAxis(dpl::face_cubic<face_idx0> f0 = {}, dpl::face_cubic<face_idx1> f1 = {}) {
      ...
      if (
        ...
        this->Face(f1).gridlines_actor_->GetVisibility())
        axes_box_.colors_->InsertNextTuple(dpl::vector3d{255, 255, 255}); // color per vtk cell, e.g. line
      else {
        axes_box_.colors_->InsertNextTuple(dpl::vector3d{255, 255, 255});

        dpl::vector2d viewport_coord[2];
        ...
        if (static constexpr bool flip = dpl::vector3i{run_dim}.cross(f0.normal).dot(f1.normal) < 0;
        ...
        auto viewport_tick_dir = dpl::vector2d{-viewport_axis_dir.y(), viewport_axis_dir.x()};
        ...
        dpl::vector2i label_size;
        
        {
          r[run_dim] = min_[run_dim];
          coord->SetValue(r);
          dpl::vector2d t0 = coord->GetComputedDoubleViewportValue(renderer_);

          r[run_dim] += step_[run_dim];
          coord->SetValue(r);
          dpl::vector2d t1 = coord->GetComputedDoubleViewportValue(renderer_);

          auto first_tick_step_viewport = t1 - t0;

          r[run_dim] = min_[run_dim] + step_[run_dim]*(count - 2);
          coord->SetValue(r);
          dpl::vector2d t2 = coord->GetComputedDoubleViewportValue(renderer_);

          r[run_dim] += step_[run_dim];
          coord->SetValue(r);
          dpl::vector2d t3 = coord->GetComputedDoubleViewportValue(renderer_);

          auto last_tick_step_viewport = t3 - t2;

          auto tick_step_viewport = dpl::vector2d{
...

        {
          ...
          dpl::vector2d axis_label_pos = coord->GetComputedDoubleViewportValue(renderer_);
        
          dpl::vector2d extra_shift{0};
    ...
    dpl::vector_n<std::string, 3> titles_ = {"X-axis", "Y-axis", "Z-axis"};
  ...
  public:
    ...
    void Init(vtkRenderer* renderer, const StartupSettings& settings, dpl::vector3d scale) {
      renderer_ = renderer;
      scale_ = scale;
      ...
      dpl::vector3d box_ticks_color = {0.6};
      ...
      for (auto& label : label_actors_) {
        ...
        text_property->SetColor(
          dpl::vector3d{0.15}
          // settings.publishing ? dpl::vector3d{0.15} : dpl::vector3d{0}
        );
        text_property->SetFontFamily(VTK_FONT_FILE);
        text_property->SetFontFile(settings.font_path.string().c_str());      
        text_property->SetFontSize(settings.visual.font_size*0.85);
      ...
      dpl::sfor<6>([&](auto i) {
        this->Face(i).Init(
          renderer,
          dpl::vector3d{0.6}
          // settings.publishing ? dpl::vector3d{0.6} : dpl::vector3d{1}
          ,
          line_width);
      });
    ...
    void SetScale(dpl::vector3d scale) {
      scale_ = scale;
    }
    
    void Build(double bounds[6], dpl::vector3d min, dpl::vector3d step, dpl::vector3i count) {
      dpl::sfor<6>([&](auto i) {
        bounds_[i] = bounds[i];
      });
