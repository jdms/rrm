...
int main( int argc, char **argv ) 
{
    ...
    status &= load_matlab_vector3<double>(s1_points, "ex4b_surface_1.mat" ); 
    status &= load_matlab_vector3<double>(s2_points, "ex4b_surface_2.mat" ); 
    status &= load_matlab_vector3<double>(s3_points, "ex4b_surface_3.mat" ); 
    status &= load_matlab_vector3<double>(sn_points, "ex4b_surface_n.mat" ); 
    ...
    SRules Surfaces; 
    Surfaces.addSurface(s1); 
    Surfaces.addSurface(s2); 
    Surfaces.defineAbove(s2); 
    Surfaces.addSurface(sn); 
    Surfaces.addSurface(s3); 
    Surfaces.removeBelow(sn); 
    Surfaces.stopDefineAbove(); 
    ...
    s1->getVertexList(s1_vlist); 
    auto face_count1 = s1->getFaceList(s1_flist); 

    s2->getVertexList(s2_vlist); 
    auto face_count2 = s2->getFaceList(s2_flist); 

    s3->getVertexList(s3_vlist); 
    auto face_count3 = s3->getFaceList(s3_flist); 

    sn->getVertexList(sn_vlist); 
    auto face_countn = sn->getFaceList(sn_flist); 
    ...
    status &= save_matlab_vector3<double>( s1_vlist, "ex4b_s1_vlist.mat" ); 
    status &= save_matlab_vector3<unsigned long int>( s1_flist, "ex4b_s1_flist.mat" ); 

    status &= save_matlab_vector3<double>( s2_vlist, "ex4b_s2_vlist.mat" ); 
    status &= save_matlab_vector3<unsigned long int>( s2_flist, "ex4b_s2_flist.mat" ); 

    status &= save_matlab_vector3<double>( s3_vlist, "ex4b_s3_vlist.mat" ); 
    status &= save_matlab_vector3<unsigned long int>( s3_flist, "ex4b_s3_flist.mat" ); 

    status &= save_matlab_vector3<double>( sn_vlist, "ex4b_sn_vlist.mat" ); 
    status &= save_matlab_vector3<unsigned long int>( sn_flist, "ex4b_sn_flist.mat" ); 
