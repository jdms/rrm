...
/// Infinity constant
#ifndef CINF
#define CINF std::numeric_limits<double>::max()
#endif
    
/// Epsilon constant
#ifndef CEPS
#define CEPS std::numeric_limits<double>::epsilon()
#endif
...
public:    
    ...
    virtual real projectPoint( const Point& p , unsigned int& index ,
        ...
        real dist = CINF, uTmp1 = 0.0, uTmp2 = 0.0 ;
        parameterU = 0.0;
        if( size() > 0 )
        {
            index = 0 ;
            ...
            pr = prTmp;
            dist = ( atBegin() - p ).norm() ;
            real distTMP = CINF , invL = 0.0 ;
            if( length()>CEPS) invL = 1.0/length();
            for( unsigned int i = 1 ; i < size() ; i++ )
            {
                uTmp1 = uTmp2;
                unsigned int iTmp = i;
    ...
    /// \return  a rank array.
    std::vector<int> douglasPeuckerRank (real tol) const {
    ...
    void douglasPeuckerSimplify (PolygonalCurve<real,dim,Point,Vector>& result, real tol) const {
        assert (size()>0);
    ...
    void douglasPeuckerDecimate (PolygonalCurve<real,dim,Point,Vector>& result, int n) const {
        assert (size()>0);
        assert (n >= 2 && n <= (int)size());
    ...
    virtual void join( const PolygonalCurve& l , real err = CEPS ){
        if( l.isEmpty() ) return;
        if( this->isEmpty()  )
        {
            this->copy(l);
        ...
        real dist = ( atEnd() - l.atBegin()).norm();
    ...
    virtual PolygonalCurve<real,dim,Point,Vector> overSketch(
           const PolygonalCurve<real,dim,Point,Vector>& curve ,
