...
void Arrangement2D::boundary(Real x, Real y, Real width, Real height)
{
    ...
    Curve_cache  segment_1;
        segment_1.supporting_curve.add(Point2D(x, height));
        segment_1.supporting_curve.add(Point2D(width, height));
        segment_1.supporting_curve.add(Point2D(width, y));
     ...
     Curve_cache  segment_2;
        segment_2.supporting_curve.add(Point2D(width, y));
        segment_2.supporting_curve.add(Point2D(x, y));
        segment_2.supporting_curve.add(Point2D(x, height));
...
bool Arrangement2D::insert ( const Curve2D& _c , Natural id )
{
...
void Arrangement2D::updateSubcurves(const std::map<Natural,std::vector<Curve2D>>& _updated_curves)
{
    for( auto& new_curves: _updated_curves )
    {
        this->curve_cache[new_curves.first].subcurves.clear();
...
// RRM interface
std::vector<Natural>Arrangement2D::getLastCurveLowerBound() const
{
    return lower_bound_;
}

std::vector<Natural>Arrangement2D::getLastCurveUpperBound() const
{
    return upper_bound_;
}

bool Arrangement2D::isAboveCurve ( Point2D _p1 , Point2D _p2 , Point2D _q1 )
{
    PolyCurve2D p;
    return (p.CCW(_p1,_p2,_q1) >= 0 );
}

void Arrangement2D::log ( )
{
