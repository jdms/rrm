	...
	class CrossSection
	{
		public:
			...
			bool initialization ( Real x , Real y, Real width, Real height )
			{
				...
				//   |----------*----------->
				segment_1 = { Point2D ( x , y ), Point2D ( width , y ), Point2D ( width , height ) };
				// (x,y)     (height,0)    (width,height)
				//   |----------*----------->
				segment_2 = { Point2D ( x , y ), Point2D ( x , height ),  Point2D ( width , height ) };
		...
		private:
			...
			bool boundary ( std::vector<Point2D> segment_1, std::vector<Point2D> segment_2 )
			{
				...
				std::vector<std::size_t> slices_indices = {1,99};
				...
				std::vector<Curve2D> segs = {c1,c2,c3};
...
//							std::cout << "Segment Indices Size   : " << curve_indices.size() << std::endl;
				...
				if ( (current_rule == GeologicRules::Sketch) )
				{
					if ( curve_all_indices.size ( ) == 2 )
					{
						...
						curve.curve.split ( curve_all_indices[0] , curve_all_indices[1] , hanging_left , normal , hanging_right );
...
//
				...
				else
				{
					...
					for ( std::size_t it = 0; it < curve_all_segments.size( ); it++)
					{
						Halfedge_handle h = d.create_loop();
						h->left_to_right = 1;
						h->opposite()->left_to_right = 0;
