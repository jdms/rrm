/*
 * CrossSectionDecorator.h
 *
 *  Created on: Oct 17, 2015
 *      Author: felipe
 */

#ifndef _MODEL_CROSSSECTION_DECORATOR_HPP_
#define _MODEL_CROSSSECTION_DECORATOR_HPP_
...
/// \brief Not enough time to learn how CGAL Decorator works ...
/// Creating RRM Decorator to suite to our needs.
/// Later, reduce this class to just a extension of CGAL::HalfedgeDS_decorator.h

namespace RRM
{
	template < class p_HDS >
	class CrossSection_Decorator: public CGAL::HalfedgeDS_items_decorator<p_HDS>
	{
		public:
		...
		public:
			...
			using CGAL::HalfedgeDS_items_decorator<p_HDS>::find_prev;
			using CGAL::HalfedgeDS_items_decorator<p_HDS>::get_prev;
			using CGAL::HalfedgeDS_items_decorator<p_HDS>::set_prev;
			using CGAL::HalfedgeDS_items_decorator<p_HDS>::get_face;
			using CGAL::HalfedgeDS_items_decorator<p_HDS>::set_face;
			using CGAL::HalfedgeDS_items_decorator<p_HDS>::get_vertex;
			using CGAL::HalfedgeDS_items_decorator<p_HDS>::set_vertex;
			using CGAL::HalfedgeDS_items_decorator<p_HDS>::get_vertex_halfedge;
			using CGAL::HalfedgeDS_items_decorator<p_HDS>::set_vertex_halfedge;
			using CGAL::HalfedgeDS_items_decorator<p_HDS>::get_face_halfedge;
			using CGAL::HalfedgeDS_items_decorator<p_HDS>::set_face_halfedge;
			using CGAL::HalfedgeDS_items_decorator<p_HDS>::set_vertex_in_vertex_loop;
			using CGAL::HalfedgeDS_items_decorator<p_HDS>::set_face_in_face_loop;
			using CGAL::HalfedgeDS_items_decorator<p_HDS>::insert_halfedge;
			using CGAL::HalfedgeDS_items_decorator<p_HDS>::remove_halfedge;
			using CGAL::HalfedgeDS_items_decorator<p_HDS>::find_prev_around_vertex;
			using CGAL::HalfedgeDS_items_decorator<p_HDS>::remove_tip;
			using CGAL::HalfedgeDS_items_decorator<p_HDS>::close_tip;
			using CGAL::HalfedgeDS_items_decorator<p_HDS>::insert_tip;
		...
		public:
			...
			CrossSection_Decorator ( p_HDS& h ) :hds(&h) {}
		...
		public:
			...
			Halfedge_handle create_center_vertex ( Halfedge_handle h )
			{
				...
				while ( g->next ( ) != hnew )
				{
					Halfedge_handle gnew = hds->edges_push_back ( Halfedge ( ) , Halfedge ( ) );
			...
			Halfedge_handle join_loop ( Halfedge_handle h , Halfedge_handle g )
			{
				...
				// edges).
				Assert_compile_time_tag ( Tag_true ( ) , Supports_removal ( ) );
				CGAL_precondition( get_face ( h ) == Face_handle ( ) || get_face ( h ) != get_face ( g ) );
		...
		protected:
			...
			void erase_connected_component_face_cycle ( Halfedge_handle h , std::vector<Halfedge_handle>& stack )
			{
		...
		private:
			...
			unsigned int tag_component ( Vertex_handle pSeedVertex , ///< one vertex of the connected component
							std::map<Vertex*, int>& tags ) ///< container holding the tag of all vertices
			{
				...
				typedef CGAL::I_HalfedgeDS_vertex_circ<Halfedge_handle, circulator_category> Halfedge_around_vertex_circulator;
				...
				while ( !vertices.empty ( ) )
				{
					...
					Halfedge_around_vertex_circulator neighbor_cir ( pVertex->halfedge ( ) ), neighbor_end = neighbor_cir;
		...
		public:
			...
			bool normalized_border_is_valid ( bool verb = false ) const
			{
				...
				CGAL::HalfedgeDS_const_decorator<p_HDS> decorator ( *hds );
	...
	void CrossSection_Decorator<p_HDS>::reorient_face ( Halfedge_handle first )
	{
	...
	CrossSection_Decorator<p_HDS>::inside_out ( Tag_false )
	{
	...
	template < class p_HDS >
	void                            // Supports: halfedge pointer in faces.
	CrossSection_Decorator<p_HDS>::inside_out ( Tag_true )
	{
		...
		{
			if ( first->is_border ( ) && first->vertex ( ) == first->opposite ( )->vertex ( ) )
				reorient_face ( first );
		}
	}
...
#endif /* _MODEL_CROSSSECTION_DECORATOR_HPP_ */
