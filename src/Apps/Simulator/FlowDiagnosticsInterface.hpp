...
class FlowDiagnosticsInterface
{
	...
	public:
		...
		///        answer: similarly as permeability so user can see different regions in different colors
		...
		///        answer: fd.txt displayed on screen or a window (some text message); fd_F and fd_Phi written to a txt file
		...
		// no need to visualise parametrilized surfaces, they are only used as a internal step for generating surface meshes.
        void getSurfaceVertices( std::vector< float >& vertices ) ;

        void getSurfaceFaces( std::vector< unsigned int >& faces ) ;
		...
		//edges of CPGelement is not available currently
