...
PlyFile *open_for_writing_ply( char *filename, int nelems, const char **elem_names, int file_type )
{
  PlyFile  *plyfile;
  char     *name;
  FILE     *fp;
...
// flist type was changed from int to unsigned int: Clarissa ( 05.01.2015 ), because I need positives integers to pass to glDrawElements.

float* compute_normals( float *vlist, int nverts, unsigned int *flist, int nfaces, int flip_sign )
{
   float x,y,z;
  float x0,y0,z0;
  float x1,y1,z1;
...
//  static int flip_sign = 0;
  ...
  float *nlist = (float *) malloc( sizeof( float ) * 3 * nverts );
  int i = 0;
  for( i = 0; i < nverts; i++) {
  ...
  for ( i = 0; i < nfaces; i++) {
    ...
    x0 = vlist[ 3*C ] - vlist[ 3*A ];
    y0 = vlist[ 3*C + 1 ] - vlist[ 3*A + 1 ];
    z0 = vlist[ 3*C + 2 ] - vlist[ 3*A + 2 ];

    x1 = vlist[ 3*B ] - vlist[ 3*A ];
    y1 = vlist[ 3*B + 1 ] - vlist[ 3*A + 1 ];
    z1 = vlist[ 3*B + 2 ] - vlist[ 3*A + 2 ];


//    x0 = vlist[ 3*A ] - vlist[ 3*B ];
//    y0 = vlist[ 3*A + 1 ] - vlist[ 3*B + 1 ];
//    z0 = vlist[ 3*A + 2 ] - vlist[ 3*B + 2 ];

//    x1 = vlist[ 3*C ] - vlist[ 3*B ];
//    y1 = vlist[ 3*C + 1 ] - vlist[ 3*B + 1 ];
//    z1 = vlist[ 3*C + 2 ] - vlist[ 3*B + 2 ];
...
//    z1 = vlist[verts[1]]->z - vlist[verts[0]]->z;
  ...
  for ( i = 0; i < nverts; i++) {
//    vert = vlist[i];
    len = nlist[ 3*i ] * nlist[ 3*i ] + nlist[ 3*i + 1 ] * nlist[ 3*i + 1 ] + nlist[ 3*i + 2 ] * nlist[ 3*i + 2 ];
    if (len == 0) {
//      vert->nx = 0;
//      vert->ny = 0;
//      vert->nz = 0;
...
PlyFile *write_ply( FILE *fp, int nelems, const char **elem_names, int file_type )
{
  PlyFile  *ply;
...
void describe_element_ply( PlyFile *plyfile, const char *elem_name, int nelems )
{
  PlyElement *elem;
