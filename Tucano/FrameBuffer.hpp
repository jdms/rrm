...
public:
    ...
    */
    void create (Eigen::Vector2i size, int num_attachs = 1)
    {
        create(size[0], size[1], num_attachs);
    }


    /**
     * @brief Deletes frambuffer and attachments.
     */
    void destroy (void)
    {
        unbind();
    ...
    */
    void readBuffer (int attach_id,  GLbyte * pixels)
    {
        ...
        pixels = new GLbyte[(int)(size[0]*size[1]*4)];
        bind();
        glReadBuffer(GL_COLOR_ATTACHMENT0+attach_id);
    ...
    void readBuffer (int attach_id,  unsigned char * pixels)
    {
        ...
        pixels = new unsigned char[(int)(size[0]*size[1]*4)];
        bind();
        glReadBuffer(GL_COLOR_ATTACHMENT0+attach_id);
        glReadPixels(0, 0, size[0], size[1], GL_RGBA, GL_UNSIGNED_BYTE, pixels);
        if (!was_binded)
    ...
    void readBuffer (int attach_id, vector<unsigned char>& pixels)
    {
        bool was_binded = is_binded;
        pixels.clear();
        pixels.resize((int)(size[0]*size[1]*4));
        bind();
        glReadBuffer(GL_COLOR_ATTACHMENT0+attach_id);
    ...
    void readBuffer (int attach_id, vector<float>& pixels)
    {
        bool was_binded = is_binded;
        pixels.clear();
        pixels.resize((int)(size[0]*size[1]*4), 0.0);
        bind();
        glFinish();
    ...
    {
        depth_values.clear();
        depth_values.resize((int)(size[0]*size[1]));
        glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
