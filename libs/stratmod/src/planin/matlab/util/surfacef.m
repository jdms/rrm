...
function [ Points, valley_bottom_complete ] = surfacef(Channel_Discretization, position, BaseSurface)
...
Coeffs = 0.4 * ones(size(p)); %+ 0.1 * (Coeffs - min(Coeffs))/(max(Coeffs) - min(Coeffs));
...
for j = 1 : numel(selected_planes),
    i = selected_planes(j); 
    y = 0.95 - Coeffs(i) * phi( x - Positions(i), zeros(size(x)), zeros(size(x)), 1/2);
