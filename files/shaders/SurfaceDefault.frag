...
void main ( void )
{
        ...
        vec4 color_t = VertexIn.color;//vec4 ( 0.75 , 0.0 , 0.0 , 1.0 );

        if ( gl_FrontFacing ) // is the fragment part of a front face?
        {
            //color_t = vec4 ( 0.75 , 0.0 , 0.0 , 1.0 );;
            newNormal = - newNormal;
        }
        else // fragment is part of a back face
        {
            //color_t = vec4 ( 0.0 , 0.75 , 0.0 , 1.0 );
        }
        ...
        outputColor = I * vec4 ( 0.0 , 0.0 , 0.0 , 1.0 ) + ( 1.0 - I ) * vec4 ( la.rgb + ld.xyz , 1.0 );
}
